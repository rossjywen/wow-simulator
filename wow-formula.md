...
把鼠标停留在弹药上会显示"增加xxx伤害每秒"，所以实际伤害需要乘以一个时间参数，而这个时间参数并不是武器的真实攻速，而是被normalize的时间，根据上面的公式可知，远程武器normalize的时间是2.8，所以这个时间参数就是2.8.

> $dmg_{弹药} = DPS_{弹药} \times 2.8$

由上面的公式可知，对于攻速低于2.8的远程武器(数值上大于)，弹药的效果低于标注的dps；而对于攻速高于2.8的远程武器，弹药的效果高于标注的dps，这就是normalize的效果。

==注意，弹药的伤害是最后计算的，不受暴击和伤害加成的影响。==
...
- ability.py 实现了技能相关代码 包括技能静态和计算公式
  ...
  - class Melee_ability 物理技能
  - class Range_ability 远程物理技能
